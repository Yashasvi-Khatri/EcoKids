...
const Signup = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: '🌱 Account created!', description: `Welcome to EcoKids, ${name}! You earned 50 welcome points.` });
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      toast({ title: '🌱 Account created!', description: `Welcome to EcoKids, ${userData.name}!` });
      navigate('/');
    } catch (error: any) {
          ...
          <CardContent>
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <Button type="submit" className="w-full text-lg py-5 rounded-xl" disabled={isLoading}>
                {isLoading ? 'Creating account...' : '🌱 Join EcoKids!'}
              </Button>
