...
const Index = () => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('ecoEducatorUser') || localStorage.getItem('edoEducatorUser') || localStorage.getItem('ecoKidsUser') || localStorage.getItem('carbonCompanionUser');
    if (stored) {
  ...
  const saveUser = (updated: ExtendedUserProfile) => {
    setUser(updated);
    localStorage.setItem('ecoEducatorUser', JSON.stringify(updated));
  };
